...
def run_geoassoc(folder_data,file_env,file_count,training_config_file,
    ...
    geoasso_model.mle['final_weights']=weights_df
    
    return dict(metadata=dict(names=names),data=dict(env=prep.covariates,count=counts,occur=occur),model=geoasso_model, train_log=logg, test_perf=(perf_hsm,perf_im))
